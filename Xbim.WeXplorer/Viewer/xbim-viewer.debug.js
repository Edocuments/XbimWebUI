...
xViewer.prototype._initKeyboardEvents = function () {
	...
	function processPressedKeys() {
		if(keysDown.length > 0)
		{
			...
			if(forward)
			{
				navigate('move', 50 * viewer.scrollSpeed, 50 * viewer.scrollSpeed);
			}
			else if(back)
			{
				navigate('move', -50 * viewer.scrollSpeed, -50 * viewer.scrollSpeed);
			...
			if(up)
			{
				navigate('pan', 0, 1.5 * viewer.panSpeed);
			}
			else if(down)
			{
				navigate('pan', 0, -1.5 * viewer.panSpeed);
	...
	function navigate(type, deltaX, deltaY) {
		...
		switch(type) {
			...
			case 'move':
				mat4.translate(transform, transform, [0, 0, deltaX]);
				mat4.translate(transform, transform, [0, 0, deltaY]);
				break;
